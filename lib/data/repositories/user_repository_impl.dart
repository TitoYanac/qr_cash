import 'dart:convert';
import 'package:qrcash/data/dao/local/preferences/preferences_user_dao.dart';
...
class UserRepositoryImpl implements UserRepository {
  ...
  @override
  Future<String> loadUserData (User user) async {
    String phone = user.personData!.uMobileID;
    try {
      ...
      if ("${responseData['Code']??''}"== user.personData!.code) {
  ...
  Future<String> updateUserImage(User user, String imageBase64) async {
    try {
      ...
      if (responseData['Message'] == 'success') {
        await loadUserData(user).then((value) => responseData['Message'] == 'success');
      }
      return responseData['Message'];
    } catch (e) {
      return 'Connection failed';
